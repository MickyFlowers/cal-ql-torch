...
def collect_episode(
    ...
    while not done and episode_steps < max_steps:
        ...
        action_norm = trainer.select_action(proprio_tensor, image_tensor, deterministic=deterministic)
...
def main(cfg: DictConfig):
    ...
    if cfg.save_every_n_epoch > 0:
        ckpt_path = os.path.join(cfg.ckpt_path, f'{cfg.logging.prefix}_online_{time.strftime("%Y%m%d_%H%M%S")}')
    ...
    for epoch in range(cfg.n_online_epochs):
        ...
        if cfg.eval_every_n_epoch > 0 and epoch % cfg.eval_every_n_epoch == 0 and epoch != 0:
            eval_metrics = evaluate_policy(
                env, td3, image_preprocessor, normalizer,
            ...
            while env_steps_this_epoch < cfg.n_env_steps_per_epoch:
                ...
                if use_random:
                    ...
                    for _ in range(min(100, cfg.n_env_steps_per_epoch - env_steps_this_epoch)):
                        ...
                        online_buffer.add(
                            proprio_norm, image, action_norm, reward,
                            next_proprio_norm, next_image, done
                        )
                    ...
                    episode_info = collect_episode(
                        env, td3, online_buffer, image_preprocessor, normalizer,
