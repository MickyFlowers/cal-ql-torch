...
class TrajSampler(object):
    ...
    def sample(self, policy, n_trajs, deterministic=False, replay_buffer=None):
        ...
        for _ in range(n_trajs):
                ...
                for i in range(len(rewards)):
                    if self.use_mc:
                        replay_buffer.add_sample(
                            observations[i], actions[i], rewards[i], next_observations[i], dones[i], mc_returns[i]
                        )
                    else:
                        replay_buffer.add_sample(
            ...
            if self.use_goal:
                traj_dict.update(dict(goal_achieved=np.array(goal_achieved_list, dtype=np.float32)))
