...
from utils.utils import concatenate_batches
...
def get_d4rl_dataset_with_mc_calculation(env, reward_scale, reward_bias, clip_action, gamma):
    ...
    dataset = qlearning_dataset_and_calc_mc(
        gym.make(env).unwrapped, reward_scale, reward_bias, clip_action, gamma, is_sparse_reward=is_sparse_reward
    )
...
def get_hand_dataset_with_mc_calculation(
    ...
    def truncate_traj(env_name, dataset, i, reward_scale, reward_bias, gamma, start_index=None, end_index=None):
        ...
        mc_returns = calc_return_to_go(
            env_name, rewards, dones, gamma, reward_scale, reward_bias, is_sparse_reward=True
        )
    ...
    if add_expert_demos:
        ...
        for i in range(len(dataset)):
            ...
            d_pos = truncate_traj(
                env_name, dataset, i, reward_scale, reward_bias, gamma, start_index=None, end_index=trunc_ind
            )
    ...
    if add_bc_demos:
        ...
        for i in range(len(dataset_bc)):
            ...
            d_pos = truncate_traj(
                env_name, dataset_bc, i, reward_scale, reward_bias, gamma, start_index=None, end_index=trunc_ind
            )
