...
def normalize(data: np.ndarray, statistics: Dict, norm_type: str, epsilon: float = 1e-6) -> np.ndarray:
...
def denormalize(data: np.ndarray, statistics: Dict, norm_type: str, epsilon: float = 1e-6) -> np.ndarray:
...
class IterativeIQLTrainer:
    ...
    def _get_policy_action(self, observation: Dict) -> np.ndarray:
        ...
        proprio_tensor = torch.tensor(proprio_norm, dtype=torch.float32).unsqueeze(0).to(self.device)
