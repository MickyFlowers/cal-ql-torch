...
class UrEnv(gym.Env):
    ...
    def __init__(self, config):
        ...
        self.admittance_controller = VelocityAdmittanceController(
            config.M, config.D, config.threshold_high, config.threshold_low
        )
        # init variables
        self.ft_value = None
        self.img_obs = None
        ...
        self.observation_space = gym.spaces.Dict(
            {
                ...
                "jnt_obs": gym.spaces.Box(
                    shape=(6,), low=-2 * np.pi, high=2 * np.pi, dtype=np.float32
                ),
                "ft_obs": gym.spaces.Box(shape=(6,), low=-np.inf, high=np.inf, dtype=np.float32),
                "img_obs": gym.spaces.Box(
                    shape=(config.image_size, config.image_size, 3), low=0, high=255, dtype=np.uint8
                ),
            }
        )
        self.running = False
        ...
        rospy.Subscriber(
            config.spacemouse_twist_topic, Twist, self._spacemouse_callback, queue_size=10
        )
        rospy.Subscriber(
            config.spacemouse_buttons_topic,
            Int8MultiArray,
            self._spacemouse_buttons_callback,
            queue_size=10,
        )
    ...
    def _control_loop(self, event):
        ...
        new_vel = self.admittance_controller.update(
            dt, target_velocity, self.ur_robot.tcp_velocity, ft_value_in_world
        )
        # print(new_vel)
    ...
    def reset(self):
        ...
        reset_pose[:3] += np.random.uniform(
            self.config.random_lower[:3], self.config.random_upper[:3]
        )
