...
def main(cfg: DictConfig):
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        pbar = tqdm(
            dataloader, desc=f"Epoch {epoch}/{cfg.num_epochs}", disable=not is_main_process()
        )
        for batch in pbar:
            # Train step
            ...
            for k in ["act/total_loss", "act/recon_loss", "act/kl_loss"]:
                if k in metrics:
                    epoch_metrics[k] += (
                        metrics[k].item() if hasattr(metrics[k], "item") else metrics[k]
                    )
            num_batches += 1
            ...
            if is_main_process():
                total_loss = (
                    metrics["act/total_loss"].item()
                    if hasattr(metrics["act/total_loss"], "item")
                    else metrics["act/total_loss"]
                )
                recon_loss = (
                    metrics["act/recon_loss"].item()
                    if hasattr(metrics["act/recon_loss"], "item")
                    else metrics["act/recon_loss"]
                )
                kl_loss = (
                    metrics["act/kl_loss"].item()
                    if hasattr(metrics["act/kl_loss"], "item")
                    else metrics["act/kl_loss"]
                )
                pbar.set_postfix(
                    {
            ...
            if (
                global_step % cfg.log_every_n_step == 0
                and is_main_process()
        ...
        if is_main_process():
            ...
            print(
                f"Epoch {epoch}: loss={epoch_metrics['act/total_loss']:.4f}, "
