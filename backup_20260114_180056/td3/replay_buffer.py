...
class MixedReplayBuffer(object):
    ...
    def sample(self, batch_size: int) -> dict:
        ...
        if offline_batch_size > 0:
            offline_indices = np.random.randint(
                0, len(self.offline_dataset), size=offline_batch_size
            )
        ...
        batch = {
            "observations": {
                "proprio": torch.stack(
                    [
                        (
                            torch.from_numpy(s["observations"]["proprio"])
                            if isinstance(s["observations"]["proprio"], np.ndarray)
                            else s["observations"]["proprio"]
                        )
                        for s in samples
                    ]
                ),
            ...
            "next_observations": {
                "proprio": torch.stack(
                    [
                        (
                            torch.from_numpy(s["next_observations"]["proprio"])
                            if isinstance(s["next_observations"]["proprio"], np.ndarray)
                            else s["next_observations"]["proprio"]
                        )
                        for s in samples
                    ]
                ),
