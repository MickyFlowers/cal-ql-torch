...
def test_model_initialization():
    ...
    assert policy_actions.shape == (
        batch_size,
        action_dim,
    ), f"Policy output shape mismatch: {policy_actions.shape}"
    assert torch.all(policy_actions >= -1) and torch.all(
        policy_actions <= 1
    ), "Policy actions not in [-1, 1]"
