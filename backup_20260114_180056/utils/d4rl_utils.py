...
def get_hand_dataset_with_mc_calculation(
    ...
    def truncate_traj(
        env_name, dataset, i, reward_scale, reward_bias, gamma, start_index=None, end_index=None
    ):
        """
    ...
    for key in dataset[0].keys():
        if key in ["agent_infos", "env_infos"]:
            continue
        concatenated[key] = np.concatenate([batch[key] for batch in dataset], axis=0).astype(
            np.float32
        )
    return concatenated
