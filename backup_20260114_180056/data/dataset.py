...
class CalqlDataset(Dataset):
    ...
    def _preload_all_data(self):
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            futures = {
                executor.submit(self._load_single_episode, args): args[0] for args in args_list
            }
        ...
        print(
            f"[CalqlDataset] Preloaded {len(self._preloaded_episodes)} episodes, {total_samples} samples"
        )
    ...
    def __getitem__(self, index):
        ...
        next_proprio = self._normalize(
            next_proprio, self.statistics["proprio"], self.config.norm_type
        )
...
class RoboMimicDataset(Dataset):
    ...
    def _load_data(self):
        with h5py.File(self.file_path, "r") as f:
            data = f["data"]
            all_demo = (
                f["mask"]["train"][:] if self.config.split == "train" else f["mask"]["valid"][:]
            )
...
class CalqlDatasetLMDB(Dataset):
    ...
    def __getitem__(self, index):
        ...
        proprio = self._normalize(
            sample["proprio"], self.statistics["proprio"], self.config.norm_type
        )
        next_proprio = self._normalize(
            sample["next_proprio"], self.statistics["proprio"], self.config.norm_type
        )
    ...
    def _setup_sample_indices(self):
        ...
        if self.sample_ratio >= 1.0:
            ...
            self.validation_sample_indices = (
                np.array(validation_indices) if validation_indices else None
            )
            ...
            print(
                f"[BCDatasetLMDB] Train: {num_episodes_to_sample}/{num_episodes} episodes "
                f"({self.sample_ratio*100:.1f}%) -> {self.num_samples} steps"
            )
            print(
                f"[BCDatasetLMDB] Val: {num_val_episodes}/{num_episodes} episodes "
...
class ACTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Normalize
        proprio = self._normalize(proprio, self.statistics["proprio"], self.config.norm_type)
        action_chunk = self._normalize(
            action_chunk, self.statistics["action"], self.config.norm_type
        )
...
class ACTDatasetLMDB(Dataset):
    ...
    def __init__(self, config: DictConfig):
        ...
        if not os.path.exists(self.lmdb_path):
            ...
            calc_statics(self.root_path, episode_files)
            self.total_samples, self.episode_lengths, self.proprio_dim, self.action_dim = (
                self._convert_to_lmdb(episode_files)
            )
        else:
            print(f"Loading from existing LMDB cache: {self.lmdb_path}")
    ...
    def __getitem__(self, index):
        ...
        action_chunk = self._normalize(
            sample["action_chunk"], self.statistics["action"], self.config.norm_type
        )
