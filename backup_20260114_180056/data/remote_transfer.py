...
class RemoteTransfer:
    def __init__(self, hostname, port, username, password=None, key_filepath=None, key_type="rsa"):
        ...
        if key_filepath:
            ...
            self.ssh.connect(
                hostname, port, username=username, pkey=pkey, look_for_keys=False, allow_agent=False
            )
        else:
    ...
    def _upload_worker(self, queue):
        ...
        while self.process.is_alive():
            ...
            remote_tmp = os.path.join(
                os.path.dirname(remote_file), f".{os.path.basename(remote_file)}.tmp"
            )
    ...
    def download_file(self, remote_file, local_file, overwrite=False):
        temp_local_path = os.path.join(
            os.path.dirname(local_file), f".{os.path.basename(local_file)}.tmp"
        )
