...
class FlowMatchingPolicyTrainer:
    ...
    def setup_lr_scheduler(self, num_training_steps):
        ...
        print(
            f"LR scheduler setup: {num_training_steps} total steps, {num_warmup_steps} warmup steps, min_lr_ratio={self.min_lr_ratio}"
        )
    ...
    def setup_multi_gpu(self, local_rank: int):
        ...
        print(
            f"[Rank {dist.get_rank()}] FlowMatching Trainer multi-GPU setup complete. Device: {device}"
        )
