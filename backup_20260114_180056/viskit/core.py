...
def to_json(stub_object):
    from rllab.misc.instrument import StubAttr, StubObject
        ...
        try:
            ...
            if disable_variant:
                ...
                try:
                    ...
                    params = load_params(params_json_path)
            exps_data.append(
                AttrDict(progress=progress, params=params, flat_params=flatten_dict(params))
            )
        except IOError as e:
            print(e)
    return exps_data
...
def extract_distinct_params(exps_data, excluded_params=("seed", "log_dir"), l=1):
    ...
    try:
        ...
        ipdb.set_trace()
    proposals = [
        (k, [x[1] for x in v]) for k, v in itertools.groupby(stringified_pairs, lambda x: x[0])
    ]
    filtered = [
        (k, v)
        for (k, v) in proposals
        if k == "version"
        or (len(v) > l and all([k.find(excluded_param) != 0 for excluded_param in excluded_params]))
    ]
    return filtered
