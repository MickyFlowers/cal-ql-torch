...
def sliding_mean(data_array, window=5):
    ...
    for i in range(len(data_array)):
        indices = list(range(max(i - window + 1, 0), min(i + window + 1, len(data_array))))
        avg = 0
        for j in indices:
            avg += data_array[j]
...
def create_bar_chart(
    ...
    plot_lists = [
        [subplot for subplot in plot_list] for plot_list in plot_lists if not plot_list[0]["x_key"]
    ]
    ...
    for y_idx, plot_list in enumerate(plot_lists):
        for idx, plt in enumerate(plot_list):
            ...
            if len(title) > 30:
                ...
                title = "<br />/".join(
                    title_parts[:-1] + [r"<b>{}</b>".format(t) for t in title_parts[-1:]]
                )
            fig["layout"]["yaxis{}".format(y_idx_plotly)].update(
                title=title,
            )
...
def get_plot_instruction(
    ...
    if len(split_keys) > 0:
        split_selectors, split_titles = split_by_keys(selector, split_keys, distinct_params)
    else:
        ...
        for plot_ind, plot_key in enumerate(plot_keys):
            ...
            for group_selector, group_legend in zip(group_selectors, group_legends):
                ...
                if (
                    ...
                    selectors = split_by_key(group_selector, best_filter_key, distinct_params)
                    scores = [
                    ...
                    if np.isfinite(scores).any():
                        ...
                        print(
                            "    best '{0}': {1}".format(
                                best_filter_key, dict(best_selector._filters)[best_filter_key]
                            )
                        )
...
def main():
    ...
    parser.add_argument("--data-filename", default="progress.csv", help="name of data file.")
    parser.add_argument("--params-filename", default="params.json", help="name of params file.")
