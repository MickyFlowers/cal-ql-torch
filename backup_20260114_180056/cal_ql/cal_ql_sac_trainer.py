...
class Trainer(object):
    def __init__(self, config, policy, qf):
        ...
        self.optimizers["log_alpha_prime"] = optim.Adam(
            self.log_alpha_prime.parameters(), lr=config.alpha_prime_lr
        )
    ...
    def setup_lr_scheduler(self, num_training_steps, warmup_ratio=0.05, min_lr_ratio=0.1):
        ...
        print(
            f"LR scheduler: {num_training_steps} total steps, {num_warmup_steps} warmup steps, min_lr_ratio={min_lr_ratio}"
        )
    ...
    def _train_step(self, batch, cql_min_q_weight=5.0):
        ...
        target_qf1_values = (
            self._get_tensor_values(
                next_observations, next_images, next_actions_temp, network=self.qf["target_qf1"]
            )
            .max(1)[0]
            .view(-1, 1)
        )
        target_qf2_values = (
            self._get_tensor_values(
                next_observations, next_images, next_actions_temp, network=self.qf["target_qf2"]
            )
            .max(1)[0]
            .view(-1, 1)
        )
        ...
        q_target = (
            rewards.view(-1, 1) + (1.0 - dones.view(-1, 1)) * self.config.discount * target_q_values
        )
        ...
        random_actions_tensor = (
            torch.FloatTensor(q2_pred.shape[0] * self.config.cql_n_actions, actions.shape[-1])
            .uniform_(-1, 1)
            .to(self.device)
        )
        curr_actions_tensor, curr_log_pis = self._get_policy_actions(
            observations, images, num_actions=self.config.cql_n_actions, network=self.policy
        )
        ...
        q1_rand = self._get_tensor_values(
            observations, images, random_actions_tensor, network=self.qf["qf1"]
        )
        q2_rand = self._get_tensor_values(
            observations, images, random_actions_tensor, network=self.qf["qf2"]
        )
        q1_curr_actions = self._get_tensor_values(
            observations, images, curr_actions_tensor, network=self.qf["qf1"]
        )
        q2_curr_actions = self._get_tensor_values(
            observations, images, curr_actions_tensor, network=self.qf["qf2"]
        )
        q1_next_actions = self._get_tensor_values(
            observations, images, new_curr_actions_tensor, network=self.qf["qf1"]
        )
        q2_next_actions = self._get_tensor_values(
            observations, images, new_curr_actions_tensor, network=self.qf["qf2"]
        )
    ...
    def _soft_update_target_networks(self):
        ...
        for target_qf, qf in [("target_qf1", "qf1"), ("target_qf2", "qf2")]:
            for target_param, param in zip(
                self.qf[target_qf].parameters(), self.qf[qf].parameters()
            ):
    ...
    def _repeat_obs_images(self, obs, images, num_repeat):
        ...
        obs_expanded = (
            obs.unsqueeze(1).expand(-1, num_repeat, -1).reshape(batch_size * num_repeat, -1)
        )
        images_expanded = (
            images.unsqueeze(1)
            .expand(-1, num_repeat, -1, -1, -1)
            .reshape(batch_size * num_repeat, *images.shape[1:])
        )
    ...
    def _get_policy_actions(self, obs, images, num_actions, network=None):
        ...
        if hasattr(network, "sample_actions"):
            actions, log_pi = network.sample_actions(
                obs, images, num_actions=num_actions, deterministic=False
            )
    ...
    def setup_multi_gpu(self, local_rank: int):
        ...
        if self.config.cql_lagrange:
            self.optimizers["log_alpha_prime"] = torch.optim.Adam(
                self.log_alpha_prime.parameters(), lr=self.config.alpha_prime_lr
            )
    ...
    def save_checkpoint(self, filepath):
        ...
        if self._use_scheduler:
            checkpoint["schedulers_state_dict"] = {
                k: v.state_dict() for k, v in self.schedulers.items()
            }
